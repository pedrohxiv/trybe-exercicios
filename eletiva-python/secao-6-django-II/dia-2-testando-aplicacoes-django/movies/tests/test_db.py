...
def test_movies_table_is_healthy():
    ...
    Movie.objects.create(
        title="Carrie",
        direction=Person.objects.create(name="Kimberly Peirce"),
    )
    number_of_movies = len(Movie.objects.all())
...
def test_movie_rooms_table_is_healthy():
    ...
    MovieRoom.objects.create(
        name="Sala 2",
        theater=MovieTheater.objects.get(id=1),
        movie=Movie.objects.get(id=1),
    )
    number_of_movie_rooms = len(MovieRoom.objects.all())
...
def test_movie_seats_table_is_healthy():
    number_of_movie_seats = len(MovieSeat.objects.all())
    assert number_of_movie_seats == 5

    MovieSeat.objects.create(
        name="A6",
        room=MovieRoom.objects.get(id=1),
    )
    number_of_movie_seats = len(MovieSeat.objects.all())
    assert number_of_movie_seats == 6
