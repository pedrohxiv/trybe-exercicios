...
describe('3 - Crie um terceiro `<fieldset>` para dados do último emprego', () => {
  ...
  it('Será verificado se o campo `Resumo do currículo` tem o tamanho máximo de 1000 caracteres.', async () => {
    const resumeTextArea = screen.getByRole('textbox', { name: resumeTextAreaName });

    await userEvent.type(resumeTextArea, superLongText);
    expect(resumeTextArea).toHaveValue(superLongText.slice(0, 1000));
  }, 100000);

  it('Será verificado se, ao digitar no campo `Resumo do currículo`, o campo recebe o valor corretamente', async () => {
    const resumeTextArea = screen.getByRole('textbox', { name: resumeTextAreaName });

    await userEvent.type(resumeTextArea, longText);
  ...
  it('Será verificado se o campo `Cargo` tem o tamanho máximo de 40 caracteres.', async () => {
    mockAlert();
    const roleInput = screen.getByRole('textbox', { name: 'Cargo' });

    await userEvent.type(roleInput, longText);
    expect(roleInput).toHaveValue(longText.slice(0, 40));
  });

  it('Será verificado que, na primeira vez (e apenas na primeira vez) em que o mouse é passado por cima desse campo, um `alert` é exibido', async () => {
    mockAlert();
    const roleInput = screen.getByRole('textbox', { name: 'Cargo' });

    expect(global.alert).not.toHaveBeenCalled();
    await userEvent.hover(roleInput);
    expect(global.alert).toHaveBeenCalledWith('Preencha com cuidado esta informação.');
    expect(global.alert).toHaveBeenCalledTimes(1);

    await userEvent.type(roleInput, 'Frontend Dev');
    ...
    await userEvent.type(roleInput, 'Frontend Developer');
  ...
  it('Será verificado se o campo `Descrição do cargo` tem o tamanho máximo de 500 caracteres', async () => {
    const roleDescriptionTextArea = screen.getByRole('textbox', { name: roleDescriptionTextAreaName });

    await userEvent.type(roleDescriptionTextArea, superLongText);
    expect(roleDescriptionTextArea).toHaveValue(superLongText.slice(0, 500));
  }, 100000);

  it('Será verificado se, ao digitar no campo `Descrição do cargo`, o campo recebe o valor corretamente', async () => {
    const roleDescriptionTextArea = screen.getByRole('textbox', { name: roleDescriptionTextAreaName });

    await userEvent.type(roleDescriptionTextArea, longText);
